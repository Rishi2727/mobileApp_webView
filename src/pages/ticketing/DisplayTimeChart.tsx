...
const getSlotState = (state: string) =>
  SLOT_STATES.find((slot) => slot.state === state) || {
    color: "#FFFFFF",
    fontColor: "#0F172A",
  };
...
const DisplayTimeChart = () => {
  ...
  useEffect(() => {
    if (
      ...
      const days = Array.from(
        { length: DesksData.catFeature.maxDaysAvailable },
        (_, i) => today.clone().add(i, "days")
      );
      setAvailableDays(days);
      if (!selectedDate || !days.some((day) => day.isSame(selectedDate, "day")))
    ...
    (dir: "left" | "right") => {
      ...
      setTimeout(() => {
        if (dir === "right" && canSlide("right")) {
          setCurrentDisplayFrom((prev) =>
            Math.min(prev + ITEMS_PER_PAGE, total - ITEMS_PER_PAGE)
          );
        }
  ...
  const formatTime = useCallback((timeSlot: string) => {
    return timeSlot.length === 8
      ...
      : moment(timeSlot).format("ll").split(",")[0].split("년 ")?.[1] ||
          moment(timeSlot).format("ll").split(",")[0].split("년 ")?.[0];
  }, []);

  // Memoized visible headers and data
  const visibleHeaders = useMemo(() => {
    return (
      DesksData?.chart?.headers?.slice(
        currentDisplayFrom,
        currentDisplayFrom + ITEMS_PER_PAGE
      ) || []
    );
  }, [DesksData?.chart?.headers, currentDisplayFrom]);
  ...
  const visibleTimeSlots = useMemo(() => {
    ...
    Object.keys(DesksData.chart.data).forEach((timeSlot) => {
      result[timeSlot] = {
        ...
        rooms:
          DesksData.chart!.data[timeSlot]?.rooms?.slice(
            currentDisplayFrom,
            currentDisplayFrom + ITEMS_PER_PAGE
          ) || [],
      };
  ...
  const handleSlotPress = useCallback(
    (timeSlot: string, room: roomChart) => {
      if (!DesksData?.catFeature) return;
      if (
        !(
          DesksData.catFeature.multiUserBooking === false &&
          DesksData.catFeature.timePicker === false
        )
      ) {
        navigate(
          `/ticketing/TimeAndUserPicker?roomCode=${roomCodes.join(
            ","
          )}&catCode=${catCode}&title=${
            room.roomName
          }&timeSlot=${timeSlot}&selectedRoomCode=${
            room.roomCode
          }&selectedDate=${selectedDate?.format(
            "YYYY-MM-DD"
          )}&slotStartTime=${room.timeStart.format("YYYY-MM-DD HH:mm:ss")}`
        );
        return;
      }
      newAlert({
        disableOnClick: true,
        message:
          t("displayTimeChart.areYouSureBookPrefix") +
          " " +
          t(room.roomName) +
          t("displayTimeChart.areYouSureBookSuffix"),
        icon: "question",
        buttons: [
          {
            title: t("common.yes"),
                ...
                buttons: [
                  {
                    ...
                    action: () =>
                      navigate(
                        `/bookings?bookingId=${result.bookingId}&catCode=${catCode}`
                      ),
                    closeOnSuccess: true,
                    color: "primary",
                  },
            ...
            action: async () => {
              const bk = await createBooking({
                ...
                type: "ROOM",
                bookingStartFrom:
                  room.timeStart && room.timeStart.isAfter(moment())
                    ? room.timeStart.format("YYYY-MM-DD HH:mm:ss")
                    : undefined,
                bookingStartFromNow:
                  !room.timeStart || room.timeStart.isBefore(moment()),
              });
    ...
    <div className="h-[92vh] bg-primary-100">
      <MyBreadcrumb
        items={
          ...
          metadata.ticketingDisplayTimeChart?.breadcrumbItems ||
          []
        }
        title={
          breadcrumbItems[catCode as BreadcrumbKeys]?.title || "Time Chart"
        }
        showBackButton={true}
      />
      {!DesksData ? (
        ...
        <div className="timeSelection p-3 bg-gray-50 flex flex-col max-h-[80vh] overflow-y-auto ">
          {/* Days Row */}
          <div
            className="mb-2"
            style={{
              display: DesksData?.catFeature.dayWiseBooking ? "none" : "block",
            }}
          >
                ...
                {availableDays.map((day) => (
                  <div key={day.format("YYYY-MM-DD")}>
                    <button
                      ...
                      className={`w-[40px] h-[40px] rounded-full border flex items-center justify-center transition-colors shadow-sm ${
          ...
          {!(
            DesksData &&
            DesksData.chart &&
            DesksData?.chart?.headers &&
            DesksData?.chart?.data
          ) ? (
            <div
              ...
              {pageLength > 2 && (
                <div className="flex items-center gap-1 mb-2">
                  {[...Array(pageLength)].map((_, index) => {
                    const isActive =
                      index === Math.ceil(currentDisplayFrom / ITEMS_PER_PAGE);
                    return (
                      <div
                        key={index}
                        className="h-1 flex-1 rounded-full"
                        style={{
                          backgroundColor: isActive ? "#93C5FD" : "#DBEAFE",
                        }}
                      ></div>
                    );
                  })}
                  ...
                  <ScrollArea className="h-full">
                    ...
                    <div className="grid grid-cols-[100px_1fr] gap-1 mb-0.5 sticky top-0 bg-gray-50 z-10">
                      <div className="rounded border border-gray-300 bg-gray-100 flex items-center justify-center p-2 shadow-sm h-[40px]">
                        <Text className="text-center font-bold text-primary text-[10px]">
                      ...
                      <div
                        className="grid"
                        style={{
                          gridTemplateColumns: `repeat(${visibleHeaders.length}, 1fr)`,
                        }}
                      >
                    ...
                    {Object.keys(visibleTimeSlots).map((timeSlot) => (
                      <div
                        key={timeSlot}
                        className="grid grid-cols-[100px_1fr] gap-1 rounded bg-white mb-0.5"
                      >
                        ...
                        >
                          {visibleTimeSlots[timeSlot]?.rooms?.map(
                            (room: roomChart, roomIndex: number) => {
